...
## Live control-plane repair (0.1.1, 2026-09-16)

The earlier "no real account" statements describe the original 0.1.0 delivery. For 0.1.1, the existing isolated Linux peer login was used against real Graph: AppFolder resolution, bucket creation, list, PUT, GET with byte comparison and DELETE were exercised. An empty permanentDelete POST failed with 411; after adding Content-Length: 0, Graph initially returned 400 invalidRequest / API not found. The strict recycle-bin fallback then returned 204. A subsequent release-binary doctor run returned 204 directly for permanentDelete, so permanent account incompatibility is **not** established.

Two temporary identities on this Linux host authenticated and exchanged bidirectional ping/pong via real Graph. They and their cloud objects were cleaned up. This is stronger than a mock test, but is not the user's physical Windows/Linux round trip. Windows must replace its executable, run doctor and mutually trust the Linux fingerprint. The Windows production machine's original failure stage remains inferred until its new diagnostic output is checked.

114 local default Rust tests, 19 Python tests, the explicitly enabled native update-process test, format and Clippy passed. New HTTP framing and failed-read preservation tests failed on the old code and passed with the fix. Full evidence and delivery details: [TD-20260916-07/08](incidents.md). Only the experimental peer executable/state and dedicated cloud control namespace were used; the stable daemon was not restarted.

0.1.1 delivery: source [c2776f1](https://github.com/LumiaBlack51/twodrive/commit/c2776f10148ee6b90dd65b13023281e04537ece6), [native CI](https://github.com/LumiaBlack51/twodrive/actions/runs/35074280918) successful on both platforms. Windows: 72 default tests plus native updater-process test, release version/health/identity checks. Windows artifact 10436929068; exe SHA-256 `84b33e056bc417c574b6016ec3d72376f264e38680ce9f36c8170fe5b14187b0`. Local Linux release SHA-256 `8f03fba31b94a3a59f7f58e7e3466b4a07d88f6e91be49775319b21057786d2e`. Distribution is manual replacement, not a newly signed GitHub auto-update Release. Follow-up documentation does not change executable inputs.
