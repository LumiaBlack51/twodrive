...
文件和目录的普通读写执行权限保存在本机并在挂载中生效，支持可执行文件和 `chmod`，重挂载后仍保留，但不会同步到 OneDrive。新导入的云端文件和旧数据库中未设置权限的条目默认为文件 0644、目录 0755。尚不保留完整 POSIX 所有权、特殊权限位及目录时间戳语义；挂载根目录的权限固定。当前客户端操作已登录用户的 `/me/drive`，不要据此假定支持多账户管理或任意 SharePoint 文档库。
