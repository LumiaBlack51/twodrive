...
| Nautilus 操作 | 实际行为 |
| --- | --- |
| **Always keep on this device（始终保留）** | 设置显式固定策略，下载现有适用文件；文件夹递归处理，固定内容不受缓存释放影响。 |
| **Cancel always keep on this device（取消始终保留）** | 移除该项目的显式策略，保留缓存；继承的父目录策略仍可能生效。 |
| **Release space（释放空间）** | 移除可释放的未固定缓存，不删除 OneDrive 项目，占位视图仍可见。 |
...
**下载过程中：** 释放未固定文件会取消下载并清理未完成片段，文件夹操作覆盖子项。旧句柄不会重启传输，主动重新打开文件可再次下载。阻塞中的网络请求需返回或超时后才能结束取消。

**写入或上传过程中：** 释放请求会持久化，显示“云朵＋同步”。上传成功且文件句柄关闭后才移除缓存；失败或冲突时保留数据。固定内容仍受保护，选择“始终保留”会取消待执行的释放请求。
