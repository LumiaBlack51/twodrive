...
从 [Releases](https://github.com/LumiaBlack51/twodrive/releases/latest) 下载 `.deb` 及配套 `.deb.sha256`，在下载目录内先校验再安装。以下以 `0.2.6-1` 为例，请替换为实际文件名：
...
TwoDrive 是使用 **OAuth 2.0 Authorization Code + PKCE** 的公共桌面客户端。需要注册自己的应用，**不要创建 client secret**。
...
编辑 `~/.config/twodrive/config.toml` 中已有的 `[graph]` 节，不要追加重复节：
...
其他配置可参考 [config.example.toml](../config.example.toml)。部分字段尚未接入实际运行逻辑，详见[当前限制](usage.zh-CN.md#当前限制与安全边界)。
